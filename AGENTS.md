...
- **Keep source files under ~500 lines.** Not a lint gate — a review
  expectation. Past the line a file is usually doing several jobs and should be
  split along those seams, one concern per file: `events.ts` at 891 lines became
  `events` / `reduce` / `reduce-helpers`, and `packages/server/src/mastra/index.ts`
  at 675 became a 97-line wiring file plus `routes/threads|controller|workspace`.
  Split by what the code *serves*, not by line count — three cohesive 200-line
  files beat six arbitrary 100-line ones. Barrel/index files are still banned:
